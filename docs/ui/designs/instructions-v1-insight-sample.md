...
> **呢份係樣本／模板**（Agent C 撰寫，2026-07-26）。app 匯出 `kind: insight` 圖文包時會自動生成一份，`insight_id`／`sketch_id`／`sketch_origin`／路徑／標題會填實際值。
> **模板版本號要同時寫兩處**：本檔第一行 HTML 註釋 `<!-- instructions.v1 -->`，同 `meta.yaml` 嘅 `instructions_template: instructions.v1`。
> **實作者要替換嘅位**：`{{sketch_dir}}`（例如 `data/sketches/workshop/sketch-20260725-02/`）、`{{sketch_id}}`、`{{sketch_origin}}`、`{{insight_id}}`（**app 喺匯出嗰刻預留，唔係由 AI 揀**）、`{{title}}`、四張圖嘅檔名同 timeframe。生成時要刪走本 blockquote。
...
1. **逐張圖，對住嗰格嘅 `owner_view` 睇。** 洞察通常只集中喺其中一兩張圖——**唔好因為有四張圖就以為佢講緊四件事**。
2. `indicators_shown` 告訴你 Owner 當時睇緊邊幾條線。**唔喺呢個清單嘅指標，Owner 冇睇過**，你唔應該基於佢嚟建構條件。
3. `indicators_shown` 空 `[]` 同缺席唔同：`[]` ＝ Owner 明確冇剔任何指標；缺席 ＝ 未知。
4. **`kind: insight` 嘅包冇 `rationale` 欄係正常嘅**——洞察嘅假設你要寫入下面 `measurement.hypothesis`，唔係搬字過紙。
