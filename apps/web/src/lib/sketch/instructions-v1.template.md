...
```
data/sketches/sketch-20260725-01/
...
1. **先鎖定 `instrument` 同 `asset_class`**——呢個係 Owner 明確選擇嘅 primary 市場（已預填：`{{instrument}}` / `{{asset_class}}`），**唔准靠圖片或標題改寫**。
...
meta:
  ...
  created: 2026-07-25
  based_on_sketch: sketch-20260725-01   # 已預填，唔准改（會替換成實際 sketch_id）
...
universe:
  primary_instrument: {{instrument}}               # 必須等於草圖 instrument，唔准改
  asset_class: {{asset_class}}    # 必須等於草圖/catalog，唔准改
  contracts: [{{instrument}}]                  # primary 必須在內；只可加入同 class 市場
...
1. **寫入**：`data/sketches/sketch-20260725-01/strategy.yaml`
